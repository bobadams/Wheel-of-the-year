...
async function fetchCity() {
  ...
  try {
    ...
    let ndvi, ndviSampLat = geo.lat, ndviSampLon = geo.lon;
    try {
      ({ ndvi, sampLat: ndviSampLat, sampLon: ndviSampLon } = await fetchModisNDVI(geo.lat, geo.lon, pct => setNdviProgress(true, pct, `MODIS NDVI: ${pct}%…`)));
    } catch {
